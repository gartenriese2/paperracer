	...
	if (!e.data.game.started) {
		...
		for (var i = 0; i < e.data.game.startingPos.length; ++i) {
			...
			if (d < dist) {
				...
				e.data.game.players[e.data.game.move].setPixelPosition(e.data.game.startingPos[i]);
			}
		...
		e.data.game.move++;
		if (e.data.game.move >= e.data.game.players.length) e.data.game.started = true;
